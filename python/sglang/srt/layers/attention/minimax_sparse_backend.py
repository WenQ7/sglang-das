...
class MiniMaxSparseAttnBackend(AttentionBackend):
    def __init__(self, runner: ModelRunner):
        ...
        self._sparse_layer_ordinal = {
            layer_id: ordinal for ordinal, layer_id in enumerate(self.sparse_layer_ids)
        }
        ...
        if self.index_topk_mode not in ("off", "reuse"):
            raise ValueError(
                "SGLANG_MINIMAX_INDEX_TOPK_MODE must be off or reuse, "
        ...
        if self.index_topk_mode != "off":
            ...
            logger.warning(
                ...
                "skip_layers=%s. This changes sparse block selection.",
                self.index_topk_mode,
        ...
        self.enable_dcp2 = os.environ.get("SGLANG_OPT_USE_MINIMAX_DCP2", "0") in (
            "1",
            "true",
            "True",
        )
        if self.dcp_size > 1:
            ...
            total_main_kv_heads = _get_minimax_num_key_value_heads(runner.model_config)
