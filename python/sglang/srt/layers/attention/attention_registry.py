...
def create_hcu_flashattention_backend(runner):
    ...
    # The release branch already routes FlashAttentionBackend through its HCU
    # interface on this platform. Keep ``hcu_fa`` as the explicit, validated
    # backend name used by existing launch configurations without maintaining a
    # second copy of the kernel adapter.
