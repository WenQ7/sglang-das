...
def minimax_sparse_prefill(
    ...
    if dcp_size > 1:
        ...
        if (
            use_msa
            or use_flash_mla_gfx938
            or use_flash_mla_gfx938_indexer
            or sink is not None
    ...
    if precomputed_topk_idx is not None:
        ...
        if precomputed_topk_idx.device != q.device:
            raise ValueError("MiniMax prefill reused top-k must be on the query device")
    ...
    q_for_main = (
        dcp_group.all_gather(q.contiguous(), dim=1).contiguous() if dcp_size > 1 else q
    )
    ...
    if dcp_size > 1:
        o, local_lse = o
        o = cp_lse_ag_out_rs_mha(o, local_lse, dcp_group, use_reduce_scatter=True)
    if return_topk_idx:
...
def minimax_sparse_decode(
    ...
    if dcp_size > 1:
        ...
        if (
            dense_main_attn_fn is not None
            or use_msa
            or use_flash_mla_gfx938
            or use_flash_mla_gfx938_indexer
        ):
