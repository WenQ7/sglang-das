...
def _build_load_aware_probabilities_kernel(
    ...
    counts = tl.load(
        expert_counts_ptr + logical_offsets, mask=logical_mask, other=0.0
    )
    num_valid = tl.load(
    ...
    tl.store(
        probabilities_ptr
        + probability_logical * max_copies
        + probability_copy,
        initial_probability,
        mask=probability_mask,
    )
    ...
    rank_loads = tl.sum(
        tl.where(fixed_mask, counts[None, :], 0.0), axis=1
    )
    ...
    for step in tl.static_range(0, BLOCK_REPLICATED):
        ...
        logical_id = tl.sum(
            tl.where(selected_mask, replicated_ids, 0), axis=0
        ).to(tl.int32)
        logical_count = tl.sum(
            tl.where(selected_mask, replicated_counts, 0.0), axis=0
        )
        ...
        candidate_allocations = tl.where(
            ...
            * logical_count
            / tl.maximum(allocation_sum, 1.0e-8),
        ...
        copy_probability = per_rank_allocation / tl.maximum(
            per_rank_multiplicity, 1.0
        )
