...
def _fp8_lm_head_top1_partials_kernel(
    ...
    for k_start in range(0, k, BLOCK_K):
        ...
        weight = tl.load(
            weight_ptr + offs_n[None, :] * stride_wn + current_k[:, None] * stride_wk,
...
def fp8_lm_head_top1_fused(
    ...
    *,
    ...
    output_values = torch.empty((m,), dtype=torch.float32, device=hidden_states.device)
    output_indices = torch.empty((m,), dtype=torch.int32, device=hidden_states.device)
