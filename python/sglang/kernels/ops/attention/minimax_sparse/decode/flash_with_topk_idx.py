...
def _decode_score_kernel(
    ...
    for i in range(chunk_start, chunk_end, BLOCK_SIZE_N):
        ...
        if DCP_SIZE > 1:
            ...
            qk += tl.where(
                off_n[None, :] < chunk_end - i, 0, float("-inf")
            )
        # [H, D], [D, N] -> [H, N]
...
def _decode_multi_q_score_kernel(
    ...
    chunk_end_block = tl.minimum(
        chunk_start_block + chunk_size_blocks, max_num_blocks
    )
    if chunk_start_block >= chunk_end_block:
        return
    chunk_start = chunk_start_block * block_size
    ...
    sid = (
        tl.load(slot_ids + first_flat_b).to(tl.int64) + max_slots
    ) % max_slots
    r2t_base = req_to_token_ptr + sid * stride_r2t_b
    ...
    for i in range(chunk_start, chunk_end, BLOCK_SIZE_N):
        ...
        if SCORE_TYPE == "max":
            ...
            block_score = sub_max + tl.log2(
                tl.sum(
                    tl.exp2(block_score - sub_max[:, :, None]), axis=2
                )
            )
        ...
        is_local = (
            curr_block_idx[None, :] >= local_start[:, None]
